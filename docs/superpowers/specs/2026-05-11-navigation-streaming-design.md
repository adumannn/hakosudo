...
|---|---|---|---|
| `/` (signed-in) | Masthead, eyebrow/vol line, CityPicker (if shown), TodayCard + YouTodayPanel hero, casual 4-tier band, bottom-strip "global pace" + leaderboard preview (these all use already-cached `getTodaySealBundle` + `getPublicDailySnapshot`) | YearScroll + freezePrompt logic — moves the 5 per-user year-range queries into an `async <YearAndStreakSection>` component | Year queries are the only uncached, per-user, range-scanning fetches on this page. Everything else is fast or cached. |
